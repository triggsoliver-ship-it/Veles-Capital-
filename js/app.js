...
/* Plus progressive accessibility enhancements applied site-wide:
   ...
   - keeps the mobile nav toggle's aria-expanded state in sync
   These run on every page because every page already loads this file via
...
(function(){
  ...
  function injectStyles(){
    ...
    var css='.cc-banner{position:fixed;left:16px;right:16px;bottom:16px;z-index:1200;max-width:720px;margin:0 auto;background:rgba(8,26,18,.97);backdrop-filter:blur(12px);border:1px solid rgba(200,162,74,.32);border-radius:14px;padding:18px 20px;display:flex;gap:14px 18px;align-items:center;flex-wrap:wrap;justify-content:space-between;box-shadow:0 20px 50px rgba(0,0,0,.45);transform:translateY(160%);transition:transform .45s cubic-bezier(.2,.7,.2,1)}'
      ...
      +'.skip-link{position:absolute;left:-9999px;top:0;z-index:2000;background:#c8a24a;color:#0c2a1f;padding:10px 18px;border-radius:0 0 10px 0;font-weight:600;text-decoration:none}'
  ...
  function enhanceA11y(){
    injectStyles();
    // Promote the primary content region to a <main id="main"> landmark if none exists.
    var main=document.querySelector('main, #main');
    if(!main){
      var hero=document.querySelector('.hero, .page-hero');
      var target=hero||document.querySelector('section');
      if(target){
        var wrap=document.createElement('main');
        wrap.id='main';
        wrap.setAttribute('tabindex','-1');
        target.parentNode.insertBefore(wrap,target);
        // Move every element from the landmark start down to (but not including) the footer.
        var node=target, footer=document.querySelector('.footer, footer');
        while(node && node!==footer){ var next=node.nextSibling; wrap.appendChild(node); node=next; }
        main=wrap;
      }
    } else if(!main.id){ main.id='main'; }
    if(main && !main.hasAttribute('tabindex')) main.setAttribute('tabindex','-1');
    // Inject a skip link as the first focusable element on the page.
    if(main && !document.querySelector('.skip-link')){
      var skip=document.createElement('a');
      skip.className='skip-link'; skip.href='#'+main.id; skip.textContent='Skip to content';
      document.body.insertBefore(skip, document.body.firstChild);
      skip.addEventListener('click',function(e){ e.preventDefault(); main.focus(); main.scrollIntoView(); });
    }
    // Label primary landmarks for assistive tech.
    var nav=document.querySelector('.nav'); if(nav && !nav.getAttribute('aria-label')) nav.setAttribute('aria-label','Primary');
    var foot=document.querySelector('.footer'); if(foot && foot.tagName==='FOOTER' && !foot.getAttribute('aria-label')) foot.setAttribute('aria-label','Footer');
    // Keep the mobile menu toggle's expanded state in sync for screen readers.
    var toggle=document.querySelector('.nav-toggle');
    var links=document.querySelector('.nav-links');
    if(toggle && links){
      if(!toggle.hasAttribute('aria-expanded')) toggle.setAttribute('aria-expanded','false');
      toggle.setAttribute('aria-controls', links.id || (links.id='nav-links'));
      toggle.addEventListener('click',function(){
        // Defer so the nav script's class toggle has already run.
        setTimeout(function(){ toggle.setAttribute('aria-expanded', links.classList.contains('open')?'true':'false'); },0);
      });
    }
  }
  function init(){
